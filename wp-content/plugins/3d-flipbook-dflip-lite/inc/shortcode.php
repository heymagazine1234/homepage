...
class DFlip_ShortCode {
  ...
  public function book( $attr, $content = '', $multi = false ) {
    ...
    //deep-link
    $atts['data-title'] = sanitize_title( $title );
    if ( !$multi && isset( $attr['slug'] ) && !empty( $attr['slug'] ) ) {
      $atts['slug'] = sanitize_title( $attr['slug'] );
    } else if ( $share_slug == 'true' ) {
      $atts['slug'] = get_post( $post_id )->post_name;
    }
    $atts['_slug'] = get_post( $post_id )->post_name;
    ...
    if ( !isset( $attr['thumb'] ) && $thumb_url !== '' ) {
      $atts['thumb'] = esc_attr( $thumb_url );
    }
    
    if ( !isset( $attr['thumbtype'] ) ) {
      $atts['thumbtype'] = esc_attr( $thumb_tag_type );
    }
    ...
    if ( isset( $attr['data-page'] ) ) {
      $atts['data-page'] = esc_attr( $attr['data-page'] );
    }
